...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    try:
        ...
        f_val = float(result["faithfulness"]) if "faithfulness" in result else (float(df["faithfulness"].mean()) if "faithfulness" in df else 0.0)
        ar_val = float(result["answer_relevancy"]) if "answer_relevancy" in result else (float(df["answer_relevancy"].mean()) if "answer_relevancy" in df else 0.0)
        cp_val = float(result["context_precision"]) if "context_precision" in result else (float(df["context_precision"].mean()) if "context_precision" in df else 0.0)
        cr_val = float(result["context_recall"]) if "context_recall" in result else (float(df["context_recall"].mean()) if "context_recall" in df else 0.0)
